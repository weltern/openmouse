...
function renderControl(): void {
  ...
  document.querySelector<HTMLButtonElement>("#haptic-enabled-toggle")?.addEventListener("click", (event) => {
    const enabled = (event.currentTarget as HTMLButtonElement).getAttribute("aria-checked") !== "true";
    void applyLogitechSetting(`${enabled ? "Enabling" : "Disabling"} haptic feedback`, async (client) => {
      await client.setHapticEnabled(enabled);
      // Only buzz on the way on; a confirmation pulse for "off" is absurd.
      // Options+ uses a different effect here than for a strength change.
      if (enabled) {
        await client.playHapticEffect(HAPTIC_EFFECTS.enableConfirmation).catch(() => undefined);
      }
    });
  });

  document.querySelector<HTMLButtonElement>("#haptic-battery-toggle")?.addEventListener("click", (event) => {
    const enabled = (event.currentTarget as HTMLButtonElement).getAttribute("aria-checked") !== "true";
    void applyLogitechSetting(`${enabled ? "Enabling" : "Disabling"} haptic battery saving`,
      (client) => client.setHapticBatterySaving(enabled));
  });

  document.querySelector<HTMLInputElement>("#name-input")?.addEventListener("input", (event) => {
    // Keeps the counter and the button honest while typing, without needing a
    // status object — everything it needs is already on the field itself.
    const input = event.currentTarget as HTMLInputElement;
    const typed = input.value.trim();
    const apply = document.querySelector<HTMLButtonElement>("#name-apply");
    if (apply) apply.disabled = typed.length === 0 || typed === input.dataset.current;
    setText("#name-remaining", `${typed.length}/${input.maxLength}`);
  });

  document.querySelector<HTMLButtonElement>("#name-apply")?.addEventListener("click", () => {
    const input = document.querySelector<HTMLInputElement>("#name-input");
    const name = input?.value.trim() ?? "";
    if (!name) return;
    void applyLogitechSetting(`Renaming the mouse to ${name}`,
      (client) => client.setFriendlyName(name));
  });
  ...
  document.querySelector<HTMLButtonElement>("#refresh-buttons")?.addEventListener("click", () => {
    void refreshButtons();
  });
...
function showStatus(status: MouseStatus): void {
  ...
  renderHostSettings(status);
  renderNameSettings(status, settingsPending);
...
function renderButtons(controls: ReprogrammableControl[]): void {
  ...
  const refresh = document.querySelector<HTMLButtonElement>("#refresh-buttons");
  if (refresh) refresh.disabled = settingInProgress;
  // Reading the button table costs far more than a status poll, so it is not
  // on the five-second refresh. Say so, rather than letting the card look live.
  setText("#button-note", divertedCount
    ...
    : "Each button can be made to act as another button on this mouse. This card is read when you connect and after each change — use Re-read to check the mouse now.");
...
function renderHapticSettings(status: MouseStatus, settingsPending: boolean): void {
  ...
  const setToggle = (selector: string, value: boolean | null | undefined): void => {
    const button = document.querySelector<HTMLButtonElement>(selector);
    if (!button) return;
    const on = value === true;
    button.setAttribute("aria-checked", String(on));
    ...
    button.style.color = on ? "#8be3a9" : "#8b8b90";
    button.style.borderColor = on ? "rgb(105 210 141 / 45%)" : "#3a3a3f";
    button.disabled = settingsPending || value == null;
  };
  setToggle("#haptic-enabled-toggle", status.hapticEnabled);
  setToggle("#haptic-battery-toggle", status.hapticBatterySaving);

  setText("#haptic-value", enabled ? match ?? String(intensity) : "Off");
}

/**
 * Rename card. The field is left alone while it has focus, so the five-second
 * poll cannot overwrite what someone is halfway through typing.
 */
function renderNameSettings(status: MouseStatus, settingsPending: boolean): void {
  const card = document.querySelector<HTMLElement>("#name-settings");
  if (!card) return;

  const current = status.friendlyName;
  const maxLength = status.friendlyNameMaxLength;
  card.style.display = current == null || maxLength == null ? "none" : "";
  if (current == null || maxLength == null) return;

  const input = document.querySelector<HTMLInputElement>("#name-input");
  const apply = document.querySelector<HTMLButtonElement>("#name-apply");
  if (input) {
    input.maxLength = maxLength;
    // The typing handler compares against this to decide whether Rename does
    // anything, so it has to travel with the field rather than be looked up.
  ...
  setText("#name-remaining", `${typed.length}/${maxLength}`);
}

/**
 * Easy-Switch card. Read-only on purpose: the slots are shown as status, not
 * as buttons, because switching lives in 0x1814 and would disconnect the mouse
 * from this computer. A control that looks clickable and disconnects you is
 * worse than no control.
 *
 * Slot numbers are displayed one-based to match the mouse's own indicator and
 * the button underneath it; the protocol counts from zero.
 */
function renderHostSettings(status: MouseStatus): void {
  const card = document.querySelector<HTMLElement>("#host-settings");
  if (!card) return;
...
/**
 * Waits for a background status poll to finish instead of dropping the click.
 * The poll runs every five seconds and takes real time over the air, so a
 * plain `if (refreshInProgress) return` guard silently swallows a fair share
...
async function refreshButtons(): Promise<void> {
  ...
  settingInProgress = true;
  setText("#read-status", "Reading buttons from the mouse…");
  let controls: ReprogrammableControl[] | null = null;
  try {
    controls = await client.readButtons();
    setText("#read-status", "Buttons read from the mouse");
  } catch (error) {
    setText("#read-status", error instanceof Error ? error.message : "Unable to read the buttons.");
  } finally {
    settingInProgress = false;
